...
| `favoriteTeamId` | `teamId` 또는 null | 예 | 선택 팀. null은 정상적인 "팀 없음" 상태이며, 이 문서가 원본이고 `shared_preferences` 는 첫 렌더용 캐시일 뿐이다 |
| `defaultThemeFamily` | `a` 또는 `b` | 예 | 팀 없음 상태에서 복원할 기본 테마 계열 |
| `brightnessPreference` | `auto` `light` `dark` | 예 | 시간대 자동 밝기 또는 사용자의 수동 밝기 선택 |
| `joinedAt` | timestamp | 예 | 가입 시각 |
| `updatedAt` | timestamp | 아니오 | 마지막 수정 시각 |
| `board` | map | 예 | 배지 판의 칸별 요약 (아래 절) |

팀을 고르면 그 팀의 테마가 최종 테마보다 우선하지만 `defaultThemeFamily` 는 그대로
보존한다. 다시 팀 없음으로 바꾸면 보존한 A/B 계열이 복원된다. 별도의 프로필 색상
필드는 더 이상 계약에 없으며 새 쓰기에서는 거부된다.

사이클 3 이전 문서는 새 설정 필드가 없을 수 있다. 앱은 그런 문서를 읽을 때
`favoriteTeamId`가 없으면 null, `defaultThemeFamily`가 없으면 `a`,
`brightnessPreference`가 없으면 `auto`로 해석한다. 새 문서와 새 쓰기는 위의 현재 계약을
모두 만족해야 한다.

닉네임만 또는 응원팀만 고치는 부분 수정은 저장소 경계에서 현재 문서를 같이 읽고,
사이클 3 이전 문서에 없는 설정만 `a`/`auto`로 보충해 한 트랜잭션으로 적용한다.
이미 문서에 있거나 같은 수정에 실린 계열·밝기 값은 보충값이 덮어쓰지 않는다.
이 수정은 당시 필수였지만 현재 계약에서 제거된 예전 프로필 색상 필드
`profileThemeKey`도
같은 트랜잭션에서 삭제한다. 현재 문서의 화이트리스트에 그 필드를 다시 허용하지는
않으므로 새 문서나 이미 이전된 문서에 다시 추가하는 쓰기는 거부된다.

빈 문서는 없다 — 프로필 설정을 마친 뒤 여섯 필수 필드를 갖춘 채 한 번에 만들어진다.
...
**칸 요약을 갱신하는 경로는 도장 쓰기 하나뿐이다** (`lib/backend/user_data.dart`
의 `UserDataStore.writeStamp`, 4.2). 그 쓰기는 도장 문서와 `board.{cellId}` 한
자리를 **같은 `WriteBatch`** 로 보내고, 개수는 앱이 먼저 읽어 센 값
(`있던 개수 + 1`)을 절대값으로 적는다. `increment` 를 쓰지 않는 것은 `tier` 가
`count` 에서 파생된 값이라 개수를 모른 채로는 등급을 적을 수 없기 때문이고,
트랜잭션을 쓰지 않는 것은 그것이 서버에 닿아야 끝나서 통신이 끊긴 구장에서
완료되지 않기 때문이다. 같은 경기의 도장이 이미 있으면 그 쓰기는 **아무것도
보내지 않는다** — 도장 문서는 결정적 id 라 덮어써도 하나지만 개수는 그렇지
않으므로, 그 단락이 개수를 실제 도장과 붙들어 두는 자리다.

그 "이미 있는가"를 **통신 때문에 알 수 없는 실행**에서는 없는 것으로 보고 쓴다.
오프라인에서 캐시에 없는 문서를 읽으면 SDK 는 스냅샷 대신 `unavailable` 로 던지는데,
오프라인에서 찍는 첫 도장이 정확히 그 갈래라 거기서 멈추면 로컬 쓰기 큐에 아무것도
들어가지 않아 복구 뒤에 올라갈 것 자체가 없다. 그래도 같은 경기가 두 번 쌓이지 않는
것은 문서 id 가 결정적이고, 큐에 들어간 쓰기가 곧바로 캐시에 반영돼 같은 오프라인
구간의 두 번째 물음에는 답이 오기 때문이다(decisions.md 2026-09-05 `[M]`).
