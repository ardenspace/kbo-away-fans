...
| 일정 크롤러 | 네이버 스포츠 API에서 KBO 일정·상태·점수를 긁어 schedule.json 산출 — 크롤 창은 KST 과거 14일(`--past-days`) + 오늘 포함 미래 30일(`--days`)이라 최근 경기 결과가 산출물에 남는다, validate 통과 시에만 기존 파일 교체, `schemaVersion` 과 games 가 둘 다 같을 때만 미변경(no_change) | `content-pipeline/crawl-schedule.mjs` | CI cron(`.github/workflows/crawl-schedule.yml`)·수동 일정 갱신 |
