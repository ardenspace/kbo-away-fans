...
### 결과
- ✅ **이메일 e2e 통과**: ops 게이트 적용(맥미니 `docker compose up -d auth`) → `/auth/v1/settings`에서 `external.kakao==true`·`mailer_autoconfirm==true` 확인 → 시뮬레이터에서 이메일 가입 → `profiles` 자동생성(`test@test.com` row, `handle_new_user` 트리거 동작) DB 확인 → 게이트·세션유지 정상.
- ⏸ **카카오 = 블로커**: 콘솔 닉네임·프로필사진 동의항목 활성화했으나 `카카오계정(이메일)`은 비즈앱(사업자등록) 심사 필요 → 개인앱 활성화 불가, GoTrue email 스코프 기본요청으로 **KOE205**. **개인사업자 등록증 보유 → 비즈앱 전환 시 재개.** v1은 이메일 로그인으로 auth 검증 대체 완료. → DECISIONS 반영.

### 블로커(갱신)
- **카카오 로그인 = 비즈앱 미전환으로 보류.** 사업자등록증 있음 → 비즈앱 전환·이메일 동의항목 활성화 시 재개(별도 후속).
