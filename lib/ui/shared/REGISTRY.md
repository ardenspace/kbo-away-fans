...
|---|---|---|---|
| `TeamThemeScope` | 목적지·상대팀의 보조 맥락 색을 하위 트리에 주입하는 전환 지점 — 같은 자리에서 팀이 바뀌면 네 색을 `MotionTokens.themeShift` 동안 보간, 정지 상태에서는 목표 테마 인스턴스를 그대로 전달 | `lib/ui/shared/team_theme_scope.dart` | 경기·구장 맥락을 설명하는 배지 등 보조 요소 |
| `DdayHeader` | 다음 원정 경기 헤더 (기본 얼굴) — 오늘/D-day/빈 상태(`.empty`) 세 상태 렌더 | `lib/ui/shared/dday_header.dart` | 홈 화면 상단 |
| `PlaceCard` | 추천 장소 카드 (샤라웃 출처 뱃지 포함) — `onLikeChanged` 를 주면 `LikeButton` 이 함께 뜬다 | `lib/ui/shared/place_card.dart` | 추천 목록·미리보기 어디든 |
...
| `PlaceDetailSheet` | 장소 상세 바텀시트 (지도 진입·길안내·OS 공유 진입점) — `onLikeChanged` 를 주면 `LikeButton` 이 함께 뜬다 | `lib/ui/shared/place_detail_sheet.dart` | 장소 카드 탭 시 |
| `CategoryChip` | 앱 루트 `AppVisualTheme` accent를 따르는 맛집/방탈출/카페 등 카테고리 필터 칩 | `lib/ui/shared/category_chip.dart` | 추천 목록 필터 |
...
| `TeamThemedAppBar` | 목적지 팀 스코프와 무관하게 앱 루트 `AppVisualTheme`의 배경·전경 역할색을 쓰는 앱바 | `lib/ui/shared/team_themed_app_bar.dart` | 모든 화면 상단 |
