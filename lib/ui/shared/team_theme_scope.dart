...
/// 목적지·상대팀의 경기·구장 보조 맥락 색을 하위 트리에 주입하는 전환 지점.
///
/// 구장/팀 맥락이 있는 화면을 이 위젯으로 감쌀 수 있지만, 팀 색을 읽는 대상은
/// 배지·경기 헤더 등 맥락 보조 요소다. `CategoryChip`과 `TeamThemedAppBar`
/// 같은 앱 컨트롤은 앱 루트 `AppVisualTheme`의 역할색을 읽는다.
///
/// 같은 자리에서 [theme] 이 바뀌면 네 색을 [MotionTokens.themeShift] 동안
/// 보간해서 건너간다(다음 원정 경기가 다른 홈팀 구장으로 넘어가는 경우 등).
/// 첫 렌더와 전환이 끝난 뒤에는 목표 테마 인스턴스를 그대로 내려보내므로,
/// 정지 상태에서는 이 위젯이 없던 때와 동일하게 동작한다.
///
