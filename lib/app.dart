...
///   된다. 기다림의 상한은 `backend/user_data_firestore.dart` 의 `watchProfile`
///   이 든다 — 서버가 영영 답하지 않아도 사람이 대기 화면에 갇히지 않는다.
/// - 오류가 온보딩으로 읽히는 것은 **캐시 읽기 오류**뿐이다. 서버 스냅샷이
///   오류로 끝난 실행은 캐시 값으로 홈에 머무른다 — 이미 팀을 고른 사람을
///   통신 문제로 팀 선택에 되돌려 세우지 않는다는 뜻이고, 캐시를 남긴 이유가
///   그것이다. 캐시가 계정에 매여 있어서(같은 파일) 그 갈래에 남의 팀이 뜰
///   위험은 없다. 두 갈래가 이 자리에서는 한 모양(`AsyncError` → 온보딩)으로
///   보이지만, 서버 오류는 여기 닿기 전에 캐시 값으로 바뀌어 있다 (그 캐시가
///   비어 있던 실행은 미선택으로 확정돼 온보딩이 맞다 — 더 물어볼 길이 없다).
