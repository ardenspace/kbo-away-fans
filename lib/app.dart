...
/// - 온보딩은 선택 팀이 **없는 것으로 확정됐을 때만** 뜬다. 값의 원본은 2.4
///   부터 사용자 문서이고 기기 저장값은 첫 프레임용 캐시다 — 서버 값을 알게
///   되면 그것으로 수렴한다 (`features/team_select/selected_team.dart`).
/// - **아직 모르는 구간은 대기 화면이다.** 서버의 첫 답을 기다리는 중이고
///   캐시도 비어 있으면(= 기기를 바꿔 처음 로그인한 실행의 첫 왕복) 팀이
///   없다고 확정할 수 없다. 그 구간을 온보딩으로 읽으면 이미 팀을 고른 사람이
///   첫 왕복 내내 팀 선택 화면을 보고, 거기서 팀을 누르면 자기 팀을 바꾸게
///   된다.
/// - **그 대기 화면은 상한을 넘어 이어지지 않는다.** 상한을 두는 자리는
///   `backend/user_data_firestore.dart` 의 `watchProfile` 이고
///   (`kProfileServerConfirmGrace`, 5초), 그 안에 아무 답도 오지 않으면 그
///   기다림은 "서버를 읽지 못했다"로 끝난다 — 그러면 바로 아래 줄의 규칙이
///   그대로 적용되어 캐시가 갈래를 정하고, 캐시가 비어 있으면 온보딩이 된다.
///   상한이 지난 뒤에 진짜 답이 오면 화면은 그 값으로 수렴한다.
///   **기다림은 둘이고 둘 다 상한이 있다.** 서버 쪽이 끝나도 기기 저장 쪽이
///   끝나지 않으면 갈래가 정해지지 않으므로, 캐시 읽기에도 같은 길이의 상한이
///   서 있다 (`features/team_select/selected_team.dart` 의
///   `kCachedTeamReadTimeout`). 그쪽은 넘어도 오류가 아니라 "캐시에 값이
///   없다"로 끝난다 — 서버가 곧 답할 수 있는 실행이기 때문이다.
/// - 오류가 온보딩으로 읽히는 것은 **아는 값이 하나도 없을 때**뿐이다. 서버
///   스냅샷이 오류로 끝났거나 상한을 넘긴 실행도 캐시에 값이 있으면 그 값으로
///   홈에 머무른다 — 이미 팀을 고른 사람을 통신 문제로 팀 선택에 되돌려 세우지
///   않는다는 뜻이고, 캐시를 남긴 이유가 그것이다. 캐시가 계정에 매여 있어서
///   (같은 파일) 그 갈래에 남의 팀이 뜰 위험은 없다. 여기 닿는 `AsyncError` 는
///   그래서 캐시 읽기 실패 하나뿐이다 — 서버 쪽 오류는 여기 오기 전에 캐시
///   값으로 바뀌어 있고, 그 캐시가 비어 있던 실행은 미선택으로 **확정돼**
///   온보딩이 맞다(더 물어볼 길이 없다).
