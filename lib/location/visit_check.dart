...
/// 전부 막았지만 **다섯 번째가 없다고는 적지 않는다.**
///
/// 그리고 **인정한 한계가 정말 한계인지도 확인하십시오.** round 7 이 겹 1 에
/// "열 0 을 블록 주석으로 여는 선언은 훅 4종과 시험 665개를 전부 통과한다"고
/// 적었는데, round 8 이 그것을 실제로 지어 재어 보니 훅 하나와 파수꾼 하나가
/// 빨간불이었다(아래 겹 1 에 실측을 적어 두었다). 막지 못한다고 적는 문장도
/// 막는다고 적는 문장과 **같은 무게로** 실측해야 한다 — 낮게 적어 둔 세기는
/// 다음 사람이 그만큼 덜 믿는다.
///
...
///
///    그리고 **(b)·(c) 는 선언을 열 0 의 머리 줄로 알아본다.** 열 0 이 블록
///    주석으로 시작하는 선언은 머리로 읽히지 않아 **바로 앞 선언의 이름을
///    물려받으므로**, 아래를 [_readDeviceFix] 바로 뒤에 두면 그 둘은
///    지나간다:
///
///    ```dart
...
///    ```
///
///    머리 줄 판정에 정규식을 한 겹 더 씌우지 않고 여기에 적어 두는 쪽을
///    골랐다: 열 0 을 블록 주석으로 여는 선언은 실수로 나오는 모양이 아니라
///    위 문단이 말한 "리뷰에서 눈에 띄는" 코드의 전형이고, 그것을 잡으려고
///    머리 줄 판정을 넓히면 다음 표기가 또 나온다.
///
///    **그 갈래 자체는 열려 있지 않다.** round 7 이 이 문단을 "아래가 시험
///    665개와 훅 4종을 전부 통과한다"로 적었는데 그것이 거짓이었다 —
///    round 8 이 위 조각을 실제로 [_readDeviceFix] 바로 뒤에 두고 재었고,
///    **둘이 빨간불**이다:
///
///    · `check-no-location-upload.sh` 의 검사 4)(a) 가 exit 2 다. 그 검사는
///      블록 주석을 걷어 낸 뒤 이름을 읽으므로 `readSneaky:func` 가
///      `LOC_TOP_NAMES` 밖으로 찍힌다.
///    · 아래 (g) 의 **측위 함수 몸통 파수꾼**이 빨간불이다. 그 줄들이
///      [_readDeviceFix] 의 몸통으로 물려받아져서 몸통 표와 어긋난다
///      (39개 중 그 하나가 실패했고, (b)·(c) 를 비롯한 나머지는 초록불이다).
///
///    그러니까 이 문단이 인정하는 것은 **(b)·(c) 하나씩의 한계**이지 그
///    갈래가 열려 있다는 것이 아니다. 두 문장의 세기가 다르다 — 앞의
///    "이 파수꾼들이 막는 것" 문단이 말하는 구분과 같은 구분이다.
...
///
///    **예외는 값을 밖으로 실어 낸다.** 이 겹이 세는 것은 이 폴더가 값을
///    보낼 수 있는 **이름**뿐이라, 던져진 예외가 좌표를 들고 나가는 갈래는
///    세지 않는다. round 8 이 실측했다: [_metersBetween] 안에
///    `throw StateError('$lat1,$lng1');` 한 줄을 두면 그 값이
///    [judgeStadiumVisit] 과 [StadiumVisitChecker.check] 를 지나
///    `lib/features/badges/stadium_visit.dart` 의 `run()` 까지 올라가는데,
///    그 메서드는 `try/finally` 라 예외를 잡지 않으므로 부르는 쪽이 실 좌표
///    문자열(`Bad state: 37.5121,127.0719`)을 그대로 받는다. 그때 훅 4종이
///    전부 exit 0 이고 `flutter analyze` 도 무지적이다. **막을 필요는 없다** —
///    좌표를 예외 메시지에 넣는 것은 실수로 나오는 모양이 아니라 위 문단이
///    말한 "리뷰에서 눈에 띄는" 코드이고, 겹 3·4 를 "값이 나갈 길이 아예
///    없다"로 읽지 않도록 여기 적어 두는 것이 이 문단의 목적이다.
...
///
///      **그 이름을 읽는 규칙이 round 8 의 거부 사유였다.** 이름을 모으는
///      자리가 수식어를 `abstract` 하나로 알고 있어서 Dart 3 의 class
///      modifier 가 붙은 타입이 그 집합에 들어오지 않았고, 그래서
///      `final class`·`sealed class`·`base class`·`interface class`·
///      `abstract final class` 로 선언한 타입의 `final` 필드가 exit 2 였다
///      (이 저장소가 `lib/` 에서 그 모양을 31번 쓴다). 같은 뿌리에서
///      `typedef Parse<T> = ...` 처럼 이름 뒤에 타입 매개변수 목록이 오는
///      함수 typedef 도 읽히지 않았다. 이제 선언 머리를 읽는 규칙을 훅
///      **스크립트 안에서 한 번만 적고** 세 자리가 그 함수 하나를 쓴다.
///
...
///    낸다 — CI 가 mawk 다. round 8 이 선언 머리를 읽는 규칙을 하나로 모은
///    뒤 30갈래를 두 awk 로 다시 재어 답이 같은 것을 확인했다). 최상위 쪽 13갈래는 위에 적었고,
...
///    통이다) · 레코드 타입 필드 `final (int, int) span;`.
///
///    round 8 이 둘을 더 쟀다. **폴더가 선언한 class·enum·mixin 의 제네릭
///    인스턴스화**는 거절한다 — `class Box<T>` 를 두고 `final Box<int> b;` 를
///    적으면 exit 2 다(같은 자리에 `final Box<DeviceFix> b;` 를 적을 수 있기
///    때문이고, 그것도 exit 2 다). 이름은 받아도 타입 인자는 받지 않는다는
///    뜻이며, **함수 타입 typedef 만 여기서 갈린다**(`Parse<int>` 는
///    통과한다 — 함수 타입은 무엇으로 인스턴스화해도 값을 담지 못한다).
///    그리고 **레코드를 가리키는 typedef 필드**도 거절한다 —
///    `typedef Point = ({double lat, double lng});` 를 두고 `final Point p;`
///    를 적으면 exit 2 인데, 레코드는 값을 담는 자리이고 이 모양은
///    `(double, double)` 과 같은 것이다(저장소에 실재하는 모양이다:
///    `lib/weather/weather.dart:87`).
///
