...
///    `_` 로 시작해 다른 라이브러리에서 부를 수 없다. 그 밖의 통로는 위치
...
///    [StadiumVisitResult] 는 이유·구장 id·경기 id 세 가지뿐이다. [DeviceFix]
...
/// 없거나 그날 경기가 없으면 **좌표를 아예 읽지 않는다**(계약의 "판정을
/// 시도하지 않는다").
///
...
enum StadiumVisitReason {
  ...
  /// 그날(KST) 어느 구장에도 후보 경기가 없었다 — 좌표를 읽지 않았다.
  noGameToday,
...
/// [candidates] 중 [now] 와 같은 KST 달력 날짜에 열리는 경기들.
///
/// 계약의 첫 조건("그 구장에 그날 경기가 있고")을 재는 자리이자,
/// [StadiumVisitChecker] 가 "좌표를 물을 이유가 있는가"를 정하는 자리다.
List<StadiumVisitCandidate> candidatesOnDayOf(
  ...
  return [
    for (final candidate in candidates)
      if (kstDateOf(candidate.startsAt) == today) candidate,
  ];
}

/// 세 조건의 **순수 판정** — 좌표 in, 결과 out.
///
/// 갈래를 보는 순서가 곧 이유의 우선순위다: 그날 경기 → 권한 → 좌표 →
/// 반경 → 시간 창.
///
/// **왜 그날 경기가 권한보다 앞인가.** 경기가 없는 날에는 판정할 것 자체가
/// 없으므로 위치 계층을 아예 건드릴 이유가 없다 — [StadiumVisitChecker] 가
/// 그 순서를 그대로 따라 권한도 좌표도 묻지 않는다. 한 해의 절반쯤은 어느
...
StadiumVisitResult judgeStadiumVisit({
  ...
  final today = candidatesOnDayOf(candidates, now);
  if (today.isEmpty) {
  ...
  for (final candidate in near) {
    ...
    // 양 끝은 포함이다 — 창의 경계에 선 사람을 밖으로 밀지 않는다.
...
class StadiumVisitChecker {
  ...
  final DeviceFixReader readFix;

  /// 한 번의 판정. 좌표는 이 메서드 안에서 태어나 이 메서드 안에서 죽는다.
  ///
  /// 위치 계층을 건드리는 순서는 [judgeStadiumVisit] 의 갈래 순서와 같다 —
  /// 그날 경기가 없으면 권한도 좌표도 묻지 않고, 권한이 없으면 좌표를 묻지
  /// 않는다(계약: "위치 권한이 없으면 판정을 시도하지 않는다").
  Future<StadiumVisitResult> check({
    ...
    if (candidatesOnDayOf(candidates, now).isEmpty) {
      return const StadiumVisitResult.rejected(StadiumVisitReason.noGameToday);
    }
...
/// 인 한 다른 계층은 기기의 좌표를 손에 넣을 방법이 없다(파일 첫 문단 1번).
