...
class FirestoreUserDataStore implements UserDataStore {
  ...
  /// "읽고 쓰는 사이에 남이 끼어드는" 갈래가 여기서는 열려 있지만, 그 갈래는
  /// 적힌 것보다 훨씬 좁다. `batch.commit()` 은 부르는 즉시 로컬 캐시에
  /// 반영되므로(실측: 오프라인에서 commit 직후 읽은 사용자 문서에 이미
  /// `count: 1` 이 서 있다) 뒤이어 도는 읽기는 이미 오른 개수를 본다. 게다가
  /// [judgeStadiumVisit] 은 한 실행에 한 경기만 돌려주고, [StampAward] 의
  /// 세션 사본이 같은 경기의 두 번째 쓰기를 아예 내보내지 않는다. 그래서
  /// 실제로 개수를 잃으려면 **더블헤더 + 1차전 창이 닫히는 순간을 사이에 둔
  /// 거의 동시의 두 트리거 + 두 `writeStamp` 가 각자의 사용자 문서 읽기와
  /// 커밋 사이에서 겹치기**가 모두 서야 한다. 이 자리를 없애는 데 트랜잭션이
  /// 필요하지는 않다 — [StampAward] 안에서 이 메서드로의 호출을 직렬화하는
  /// 것으로 충분하고, 오프라인 성질도 그대로 남는다(계약 밖이라 지금은
  /// 문장으로만 적는다). 어긋나더라도 규칙이 지키는 것은 그대로다 —
