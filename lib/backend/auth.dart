...
abstract class AuthService {
  ...
  ///
  /// 화면 분기에 쓰지 않는다. "로그아웃"과 "아직 모름"이 같은 null 로 오기
  /// 때문이다 (Firebase Auth 는 네이티브가 영속 세션을 복원해 알려 줄 때까지
  /// 로그인해 둔 사람도 null 이다). 분기는 [authStateChanges] 로 한다 —
  /// 거기서는 "아직 모름"이 값의 부재로 구분된다.
...
/// 로 첫 값을 미리 채우지 않는 것은 그 값이 "로그아웃"과 "아직 모름"을 같은
/// null 로 말하기 때문이다 — Firebase Auth 는 네이티브가 영속 세션을 복원해
/// 알려 줄 때까지 로그인해 둔 사람도 null 이라, 그 null 을 첫 값으로 흘리면
/// 콜드 스타트에서 로그인 화면이 한 번 번쩍인 뒤 홈으로 바뀐다. 스트림에서는
/// "아직 모름"이 값의 부재(= 게이트의 로딩 갈래)로 구분되므로 확정되지 않은
/// 로그아웃이 화면에 뜨지 않는다.
///
