...
- `check-no-location-upload.sh` — 기기 위치는 서버에 올리지 않는다는 데이터 소유권
  ...
  두고 모양만 바꾸는 변경도 걸린다). round 11 이 그 문장을 한 번 거짓으로
  만들었다: 그 검사의 문장 훑기가 세 겹 문자열을 몰라서 아포스트로피가 든 세 겹
  문자열 한 줄 뒤를 통째로 삼켰고, 삼켜진 구간의 최상위 저장소가 조용히
  통과했다. 문자열을 알아보는 규칙을 `scripts/hooks/dart-source.sh` 한 자리로
  모으고(사본을 둘 내놓는다 — 문자열 내용이 필요한 쪽과 문자열이 어디까지인지만
  필요한 쪽), 훑기가 사본의 전제를 스스로 확인하게 해서 닫았다. round 7 이 그 방향을 뒤집었다: 그 전에는
...
변수와 클로저 캡처를 보지 않는다. 겹 1 의 파수꾼 **둘**은 선언을 열 0 의 머리
줄로 알아보므로, 열 0 을 블록 주석으로 여는 선언은 앞 선언의 이름을 물려받아
**그 둘을** 지나간다(정규식을 한 겹 더 씌우는 대신 적어 두었다 — 다만 그
갈래 자체는 훅의 최상위 이름 검사와 일곱째 파수꾼이 잡는다. round 8 실측).
그리고 겹 3 은 값을 보낼 수 있는 **이름**만 세므로, 좌표를 예외 메시지에
담아 던지면 그 값이 판정 계층 밖으로 나간다(round 8 실측: 훅 4종이 exit 0
이고 부르는 쪽이 실 좌표 문자열을 받는다). 겹별로 무엇이 지키고 무엇은 지키지 않는지는
