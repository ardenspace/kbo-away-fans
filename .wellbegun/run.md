...
- **App Check 증명 제공자 등록 — iOS 만 남는다** ([M] 2.3, 2026-09-03; 2026-09-03 정정: 안드로이드는 등록 가능했다). 안드로이드는 Play Integrity 를 콘솔에서 바로 등록했다 — Play Console 이 필요한 것은 *등록* 이 아니라 실제 *판정* 이고, 스토어 배포 전까지는 개발 빌드가 디버그 토큰으로 지나간다. 남은 벽은 iOS 뿐이다: DeviceCheck 는 Apple Developer 포털의 `.p8` 키를, App Attest 는 같은 멤버십을 요구한다 — 애플 로그인을 막는 그 벽과 같다. 지금 서는 것은 디버그 토큰뿐이고 `kakaoCustomToken` 은 강제가 켜진 채 배포되므로 **릴리스 빌드의 카카오 로그인이 서지 않는다.** 코드는 `lib/backend/app_check.dart` 가 빌드 모드로 갈라 두어 콘솔 등록만 하면 되고 새 빌드는 필요 없다. 스토어 출시 준비 시점에 애플 항목과 함께 꺼낸다.
