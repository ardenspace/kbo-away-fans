...
- **연 $99 Apple Developer Program 하나에 묶인 것들** — 아래 셋은 같은 벽이다. 멤버십을 얻는 날 **한 번에** 처리하고, 그 전까지는 어느 하나도 따로 시도하지 말 것(세 번 다시 조사하게 된다).
  ...
  3. **iOS App Check 증명 제공자** ([M] 2.3, 2026-09-03). DeviceCheck 는 Apple Developer 의 Certificates, Identifiers & Profiles → Keys 에서 만드는 `.p8` private key 를 요구하고 그 화면은 Account Holder 또는 Admin 역할을 요구한다 — 무료 Apple ID 로는 들어갈 수 없다. 그래서 iOS 는 App Check 앱 목록에 "등록되지 않음"으로 남아 있고, **iOS 릴리스 빌드의 카카오 로그인은 이 항목이 풀려야 선다**(개발 빌드는 디버그 토큰으로 지나간다). 코드는 `lib/backend/app_check.dart` 가 빌드 모드로 갈라 두어 콘솔 등록만 하면 되고 새 빌드는 필요 없다.
