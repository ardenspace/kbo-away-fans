...
- **App Check 증명 제공자 등록** ([M] 2.3, 2026-09-03). 막고 있는 것: Play Integrity 는 Play Console 등록, DeviceCheck·App Attest 는 유료 Apple Developer Program — 애플 로그인을 막는 그 벽과 같다. 지금 서는 것은 디버그 토큰뿐이고 `kakaoCustomToken` 은 강제가 켜진 채 배포되므로 **릴리스 빌드의 카카오 로그인이 서지 않는다.** 코드는 `lib/backend/app_check.dart` 가 빌드 모드로 갈라 두어 콘솔 등록만 하면 되고 새 빌드는 필요 없다. 스토어 출시 준비 시점에 애플 항목과 함께 꺼낸다.
