...
- [>] 2.3 **사람 몫 대기 — 실행 멈춤** (사용자 선택 2026-09-03: "멈추고 콘솔 먼저"). 까닭은 2.2 때와 같다: 카카오 경로가 한 번도 실물로 돌지 않았고, 특히 함수 배포는 App Check 강제가 켜진 채 나가는 것이라 디버그 토큰 절차가 어긋나면 카카오 로그인이 전부 막힌다. 코드를 더 쌓은 뒤에 그것을 만나면 움직이는 조각이 늘어난 채로 디버그하게 된다. 아래 체크리스트 14항목이 끝나야 2.4 로 간다.
