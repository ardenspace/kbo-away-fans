...
2. **Acceptance criteria:** 세 정보가 보이고 닉네임·프로필 색 변경이 Firestore 에 반영된다. 이메일을 주지 않는 제공자(카카오)로 로그인한 사람에게는 그 자리가 빈칸으로 남지 않고 제공자 표시로 대신한다. 로그아웃하면 로그인 화면으로 돌아간다. 로그인 기기 관리는 없다(non-goal). **로그아웃 진입점이 생기므로 2.2 가 확인하지 못한 두 항목을 여기서 잰다**: 로그아웃 후 다시 로그인하면 홈까지 들어간다("로그인 상태를 세우지 못했어요"가 뜨면 실패), 그리고 로그아웃 후 구글 버튼을 누르면 계정 선택 화면이 다시 뜬다(곧장 같은 계정으로 들어가면 `signOut` 의 `GoogleSignIn.instance.signOut()` 이 구글 세션을 정리하지 못한 것이다).
3. **Boundary tests:**
