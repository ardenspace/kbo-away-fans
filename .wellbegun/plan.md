...
2. **Acceptance criteria:** 설정 파일이 없는 클론에서도 빌드·analyze·테스트가 통과한다(사이클 1의 no-op 패턴 유지). 설정이 있는 기기에서 구글 로그인으로 계정이 만들어진다. iOS 에서 애플 로그인이 동작한다. Firebase 콘솔에 예산 알림과 상한이 걸려 있다.
3. **Boundary tests:**
