...
- [2026-09-01] [S] 배지 등급 3단계는 `first`/`regular`/`master`(첫 방문·단골·마스터), 임계는 한 칸당 도장 1·3·10개, 표현은 팀 색 위에 얹는 링 색·굵기 + 흰 광택 세기 — 칸 몸통이 팀 대표색이라 등급이 몸통 색을 갈아치우면 10개 팀 색마다 등급이 다르게 읽히므로 색을 더하지 않고 위에 얹는 값만 둔다(테스트가 등급 색과 팀 대표색이 겹치지 않음을 단언); 임계 1/3/10 은 한 시즌 한 구장에 서너 번 가는 팬을 기준으로 잡은 초기값이라 실사용을 보고 조정
