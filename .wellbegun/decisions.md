...
- [2026-09-01] [S] `badge.*` 에 판의 열 수를 두지 않는다 — 칸 수(10)·칸 크기·간격·빈 칸 투명도·등급 표시 크기까지만 토큰이고, 배치(격자/지도형)는 spec 의 Implementer discretion 이 판을 만드는 단계에 맡긴 결정이라 수치로 미리 못 박으면 그 재량을 이 단계가 앞질러 쓰게 된다
