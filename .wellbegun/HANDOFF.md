...
- **비용**: Cloud Functions 때문에 Firebase Blaze 전환이 필요하고 카드 등록은
  피할 수 없습니다(사용자 승인함). 무료 할당량(일 5만 읽기·2만 쓰기)은 그대로
  포함되므로 **Firestore 읽기 패턴을 그 안에 머물게 하는 것이 설계 조건**입니다.
- **배지 판 읽기 패턴은 이미 정해졌습니다.** 사용자 문서의 칸별 요약만 읽고,
  개별 도장은 칸 상세를 열 때만 읽습니다. 도장 쓰기가 요약 갱신을 같은 트랜잭션
  으로 함께 합니다. 판을 열 때마다 도장을 전부 읽는 방식은 사용자 800명에서 무료
  한도에 닿고 오래 쓴 사람일수록 비싸져서 기각했습니다(요약 방식은 2만 5천명).
- **애플 로그인**: Apple Developer Program(연 $99)은 사용자가 지불하기로 했습니다.
  무료 계정으로 Sign in with Apple capability 설정이 가능한지는 확인하지 않았으나,
  유료 가입이 확정되었으므로 진행에 걸림돌이 아닙니다.
