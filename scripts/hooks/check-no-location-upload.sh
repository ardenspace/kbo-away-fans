...
#
#
#      **작정한 우회 하나를 적어 둔다 — 두 검사의 세기가 다르다** (round 12
#      가 실측했다). 이 검사가 지시자를 알아보는 자리는 `import`/`export` 와
#      **따옴표가 같은 줄에 있을 때**만 문장을 모으기 시작한다. 그래서
#      `import` 뒤에 줄바꿈을 넣으면 그 지시자가 아예 보이지 않는다:
#
#          import
#              'package:http/http.dart';
#
#      이것은 이 헤더 첫 문단이 이미 인정한 범위 안이다(작정하고 패턴을
#      비켜 가는 표기는 막지 못한다). 그래도 적어 두는 까닭은 **짝 훅과 세기가
#      다르기 때문**이다: `check-firebase-import-boundary.sh` 는 사본을 줄
#      단위로 grep 하므로 같은 표기에서도 백엔드 SDK 를 잡는다. 실측 —
#      `lib/location/visit_check.dart` 머리에 위 두 줄 표기로
#      `package:cloud_firestore/cloud_firestore.dart` 를 넣으면 이 검사는
#      exit 0 이고 짝 훅은 exit 2 다. 같은 표기의 `package:http` 는 짝 훅의
#      패턴 밖이라 둘 다 exit 0 이다. **두 훅을 같은 세기로 읽지 마십시오.**
#
#      고치지 않은 까닭은 이 헤더 첫 문단에 적힌 것과 같다: 정규식을 한 겹 더
#      씌워도 다음 표기가 남고, 그런 코드는 리뷰에서 눈에 띈다. 값어치가 있는
#      쪽은 **실수로 지나갈 수 있는 갈래**인데, `import` 뒤의 줄바꿈은
#      `dart format` 이 되돌리는 표기라 실수로 남지 않는다(실측: `dart format`
#      이 위 두 줄을 한 줄로 되돌린다).
